...
- **Target the dev server first.** The dev server at `https://dev.andykeys.me:3001` is the canonical test environment (real DB, auth, file uploads). Write the test plan for someone SSHed into `ak-home-server` running against the dev deployment. Local Docker is a fallback only — if a step only makes sense locally, label it clearly as `# Local Docker fallback (if dev server unavailable)`.
