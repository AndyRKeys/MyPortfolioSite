...
- Authenticated admin E2E CSP scan on every deploy (#342): `backend/scripts/tests/test-admin-e2e-csp.js` mints a JWT from `JWT_SECRET`, injects it into `localStorage.adminToken`, loads `/admin/`, and drives the Nominatim forward and reverse geocode interactions. Any `securitypolicyviolation` during authenticated flows fails the check. This is the interaction path that caused the original #330 breakage. Added `check_admin_e2e_csp()` to `deploy-lib.sh`, wired into `deploy.sh`, warn-only.

### Security
- CSP `report-to` added alongside deprecated `report-uri` (#337): `Reporting-Endpoints` header now names the same `/api/debug/csp-violations` endpoint; both directives present during transition so modern and legacy browsers both deliver violation reports.
- Debug endpoint rate limiting migrated from in-memory `Map` to DB-backed `createRateLimiter` (#337): `POST /debug/errors` and `POST /debug/csp-violations` limits now survive container restarts and are consistent with the contact and auth route limiters. `docs/SECURITY.md` rate limiting table updated.
