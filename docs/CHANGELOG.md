...
- Travel CSV bulk-import now accepts photos alongside the CSV: `extract_geo_loc.py`'s export stage adds a `photos` column, and the admin panel's Bulk import UI gains a folder picker to attach them to each newly-created memory in one step (#511)
