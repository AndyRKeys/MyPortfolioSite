...
- Dead-code cleanup from the #522 code review (part of #522): removed the unused `resources/js/admin/travel/` directory (superseded by `resources/js/admin/travel.js`), the retired `SetupSchema` validator and its tests, the unused `body-parser` and `nodemon` dependencies, and dead code in `resources/js/config.js` (`isDev`, commented-out block, deprecated `API` alias). Merged `backend/utils/auditLog.js` into `backend/utils/audit.js` (both `logAudit` and `pruneAuditLog` now live in one module). Renamed `healthRouter` → `healthHandler` and dropped its unused query result binding. Simplified `buildPostCard` to blog-only (travel cards use `buildPublicTravelCard`); `travel-post.js` now uses the shared `formatVisitDate`. `login.js` no longer reads a non-existent `message` field from the magic-link response. Fixed docs drift: CLAUDE.md no longer claims jQuery is in use (removed in #389) and seed blog content now correctly says the admin JWT lives in `localStorage`.
