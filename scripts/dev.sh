...
# Mirrors deploy.sh behaviour for the Docker dev environment.
#
# Commands:
...
#
# Run from the repo root: bash scripts/dev.sh <command>
set -e
...
case "$1" in
  ...
  up)
    ...
    # just pulled (matching the deploy.sh detection pattern).
  ...
  db)
    echo "=== Opening psql shell (portfolio_dev) ==="
  ...
  *)
    echo "Usage: bash scripts/dev.sh [up|down|reset|logs|db]"
    echo ""
